@if($this->filters())
    ...
    <div class="hs-dropdown [--auto-close:true] relative inline-flex" data-role="suitable-filter">
        <!-- Filter Button -->
        <button type="button" class="py-1.5 sm:py-2 px-2.5 inline-flex items-center gap-x-1.5 text-sm sm:text-xs font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none focus:outline-hidden focus:bg-gray-50 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700"
        ...
        <!-- Dropdown -->
        <div class="hs-dropdown-menu hs-dropdown-open:opacity-100 w-80 transition-[opacity,margin] duration opacity-0 hidden z-10 bg-white rounded-xl shadow-xl dark:bg-neutral-900" role="menu" wire:ignore.self>
            <div class="p-4">
                <div class="space-y-3">
        ...
        @push('scripts')
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    const resetButtons = document.querySelectorAll('[data-role="suitable-filter"] button[wire\\:click="resetFilters"]');
                    resetButtons.forEach(button => {
                        button.addEventListener('click', function () {
                            const form = this.closest('[data-role="suitable-filter"]').querySelector('form');
                            if (form) {
                                // Reset form inputs
