    ...
    <thead>
        <tr class="border-t border-gray-200 divide-x divide-gray-200 dark:border-neutral-700 dark:divide-neutral-700">
            @foreach($columns as $column)
                @if($column->getSortableColumn())
                    <th scope="col" class="min-w-40">
                        ...
                        <div class="hs-dropdown relative inline-flex w-full cursor-pointer">
                            <button type="button"
                                    class="px-5 py-2.5 text-start w-full flex items-center gap-x-1 text-sm text-nowrap font-normal text-gray-500 focus:outline-hidden focus:bg-gray-100 dark:text-neutral-500 dark:focus:bg-neutral-700"
                ...
                @else
                    <th scope="col" class="px-5 py-2.5 text-start text-sm text-nowrap font-normal text-gray-500 dark:text-neutral-500" {!! $column->headerAttributes(asHtml:true) !!}>
                        {!! $column->getHeader() !!}
                    </th>
                @endif
            @endforeach
